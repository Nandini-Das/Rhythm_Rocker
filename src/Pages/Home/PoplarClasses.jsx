  ...
  <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3">
    {classes.map((theClass, index) => (
      <div key={index} className="card max-w-md p-4 mx-auto bg-white shadow-lg rounded-lg">
        <img src={theClass.image} alt={theClass.name} className="w-48 h-48 object-cover rounded-full mx-auto mb-4" />
        <div className="">
          <h2 className="text-xl font-bold mb-2">{theClass.name}</h2>
          <p className="text-gray-500">Instructor: {theClass.instructor}</p>
          <p className="text-gray-500">Enrolled Students: {theClass.students}</p>
          <p className="text-gray-500">Available Seats: {theClass.availableSeats}</p>
        </div>
      </div>
    ))}
  </div>
